...
class Parse5ka:
    ...
    def run(self):
        for product in self._parse(self.start_url):
            ...
            file_path = self.save_dir.joinpath(file_path)
            self._save(product, file_name)
    def _parse(self, url):
        while url:
...
class CategoriesParser(Parse5ka):
    def __int__(self,categories_url, *args, **kwargs):
        self.catgories_url = categories_url
